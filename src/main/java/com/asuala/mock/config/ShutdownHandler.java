...
public class ShutdownHandler implements ApplicationListener<ContextClosedEvent> {
    ...
    public void onApplicationEvent(ContextClosedEvent event) {
        ...
        if (MainConstant.systemInfo.getSystem().contains("LINUX")){
            ...
            try {
                int i = 0;
                while (!InotifyLibraryUtil.fixedThreadPool.awaitTermination(1000, TimeUnit.SECONDS)) {
                    if (i++ > 10) {
            ...
            try {
                int i = 0;
                while (!FileListener.fixedThreadPool.awaitTermination(1000, TimeUnit.SECONDS)) {
                    if (i++ > 10) {
